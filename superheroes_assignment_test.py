from superheroes_assignment import *
import math
import pytest
import io
import sys
...
def test_ability_instance():
    ...
    debug_ability = Ability("Debug", 100)
    assert debug_ability

def test_ability_name():
    #Test Ability Name
    debug_ability = Ability("Debug", 100)
    assert debug_ability.name == "Debug"
...
def test_ability_attack():
    ...
    for _ in range(0, test_runs):
        attack = debug_ability.attack()
        assert attack >= 0 and attack <= 1500
...
# Test Heroes Class
def test_heroes_instance():
    Fuyumi = Hero("Fuyumi")
    assert Fuyumi

def test_hero_add_ability():
    Sublime_Heaven = Ability("Sublime Heaven", 500)
    Fuyumi = Hero("Fuyumi")
    assert len(Fuyumi.abilities) == 0
    Fuyumi.add_ability(Sublime_Heaven)
    assert len(Fuyumi.abilities) == 1
    # Check for correct type
    assert "Ability" in str(Fuyumi.abilities[0])
    assert Fuyumi.abilities[0].name == "Sublime Heaven"


def test_hero_add_multi_ability():
    Sublime_Heaven = Ability("Sublime Heaven", 500)
    Ruptured_Sky = Ability("Ruptured Sky", 500)
    Fuyumi = Hero("Fuyumi")
    assert len(Fuyumi.abilities) == 0
    Fuyumi.add_ability(Sublime_Heaven)
    assert len(Fuyumi.abilities) == 1
    Fuyumi.add_ability(Ruptured_Sky)
    assert len(Fuyumi.abilities) == 2
    # Check for correct type
    assert "Ability" in str(Fuyumi.abilities[0])
    assert Fuyumi.abilities[0].name == "Sublime Heaven"
...
def test_hero_attack_ability():
    Sublime_Heaven = Ability("Sublime Heaven", 30000)
    Fuyumi = Hero("Fuyumi")
    assert Fuyumi.attack() >= 0
    Fuyumi.add_ability(Sublime_Heaven)
    attack = Fuyumi.attack()
    assert attack <= 300000 and attack >= 0


def test_hero_ability_attack_mean_value():
    Fuyumi = Hero("Fuyumi")
    strength = random.randint(10, 30000)
    Sublime_Heaven = Ability("Sublime Heaven", strength)
    Fuyumi.add_ability(Sublime_Heaven)
...
def test_hero_attack_weapon():
    ...
    for _ in range(0, test_runs):
        ...
        assert attack <= 200000 and attack >= 0
...
def test_hero_multi_weapon_attack():
    ...
    for _ in range(0, test_runs):
        ...
        assert attack <= 150000 and attack >= 0
...
def test_team_name():
    team = Team("Gold Team")
    #Gold Team Rules
    assert team.teamname == "Gold Team"
...
def test_team_remove_unlisted():
    ...
    #assert None == 0
    #I got an error, even though both assert None == 0 matched assert None == 0?
